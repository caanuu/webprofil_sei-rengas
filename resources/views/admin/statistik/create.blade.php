...
<div class="max-w-xl">
    ...
    <div class="bg-white rounded-2xl p-8 shadow-sm border border-slate-100">
        <h2 class="text-xl font-bold text-slate-800 mb-6">Tambah Data Statistik Layanan</h2>
        ...
        <form action="{{ route('admin.statistik.store') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="nama_layanan" value="{{ old('nama_layanan') }}" class="form-input @error('nama_layanan') is-invalid @enderror" placeholder="Contoh: Surat Keterangan Domisili" required>
            ...
            <div>
                ...
                <input type="number" name="jumlah_dilayani" value="{{ old('jumlah_dilayani') }}" class="form-input @error('jumlah_dilayani') is-invalid @enderror" min="0" required>
                @error('jumlah_dilayani')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
            </div>
                ...
                <div>
                    ...
                    <select name="bulan" class="form-input" required>
                        @foreach(['Januari','Februari','Maret','April','Mei','Juni','Juli','Agustus','September','Oktober','November','Desember'] as $i => $nama)
                            <option value="{{ $i + 1 }}" {{ old('bulan', date('n')) == ($i + 1) ? 'selected' : '' }}>{{ $nama }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label class="form-label">Tahun <span class="text-red-500">*</span></label>
                    <input type="number" name="tahun" value="{{ old('tahun', date('Y')) }}" class="form-input" min="2020" max="2100" required>
                </div>
            </div>
