...
@section('content')
    ...
    <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 card-hover">
            <div class="flex items-center justify-between mb-4">
                <div
                    class="w-12 h-12 bg-gradient-to-br from-emerald-500 to-emerald-700 rounded-xl flex items-center justify-center shadow-lg">
    ...
    <div class="grid lg:grid-cols-2 gap-6 mb-8">
        <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
            <h3 class="font-bold text-slate-800 mb-4"><i class="fas fa-chart-line text-blue-600 mr-2"></i>Statistik Layanan
                {{ date('Y') }}</h3>
            <canvas id="layananChart" height="200"></canvas>
        </div>

        <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
            <h3 class="font-bold text-slate-800 mb-4"><i class="fas fa-chart-doughnut text-amber-600 mr-2"></i>Status
                Pengaduan</h3>
    ...
    <div class="grid lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
            <div class="flex items-center justify-between mb-4">
                ...
                <a href="{{ route('admin.berita.index') }}"
                    class="text-sm text-blue-600 hover:text-blue-800 font-medium">Lihat Semua &rarr;</a>
            </div>
            <div class="space-y-3">
                @forelse($beritaTerbaru as $item)
                    <div class="flex items-center gap-3 p-3 rounded-xl hover:bg-slate-50 transition">
                        <div
                            class="w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 {{ $item->kategori === 'kegiatan' ? 'bg-amber-100' : 'bg-blue-100' }}">
                            <i
                                class="fas {{ $item->kategori === 'kegiatan' ? 'fa-calendar-check text-amber-600' : 'fa-newspaper text-blue-600' }} text-sm"></i>
                        </div>
                        ...
                        <span
                            class="badge {{ $item->is_published ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500' }}">
        ...
        <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
            <div class="flex items-center justify-between mb-4">
                <h3 class="font-bold text-slate-800"><i class="fas fa-headset text-amber-600 mr-2"></i>Pengaduan Terbaru
                </h3>
